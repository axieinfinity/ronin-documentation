...
```
NAME:
   ronin - the ronin command line interface
...
SUBSCRIBER OPTIONS:
  --subscriber                                       subscribes to blockchain event
  --subscriber.blockEventTopic value                 topic name that new block will be published to
  --subscriber.reOrgBlockEventTopic value            topic name that reorged block will be published to
  --subscriber.txEventTopic value                    topic name that new transactions will be published to
  --subscriber.reorgTxEventTopic value               topic name that reorg transactions will be published to
  --subscriber.kafka.partition value                 partition of kafka topic. Default 0 (default: 0)
  --subscriber.kafka.url value                       kafka connection url
  --subscriber.kafka.authentication value            authentication type. eg: PLAIN, SCRAM-SHA-256, SCRAM-SHA-512
  --subscriber.kafka.username value                  username to access kafka
  --subscriber.kafka.password value                  Password to access kafka
  --subscriber.maxRetry value                        maximum retry time for a failed job (default: 100)
  --subscriber.workers value                         number of concurrent workers (default: 1024)
  --subscriber.backoff value                         the weighted number which is used for exponential backoff that handles failed job (default: 5)
  --subscriber.publisher value                       type of publishing framework: kafka, google pub/sub
  --subscriber.fromHeight value                      the height that the program starts publishing events (default: 0)
  --subscriber.logsEventTopic value                  topic name that new logs will be published to
  --subscriber.queueSize value                       specify size of workers queue and jobs queue (default: 2048)
  --subscriber.blockConfirmedEventTopic value        topic name that confirmed block will be published to
  --subscriber.transactionConfirmedEventTopic value  topic name that confirmed transaction will be published to
  --subscriber.logsConfirmedEventTopic value         topic name that confirmed logs will be published to
  --subscriber.safeBlockRange confirmAt              confirm block that behind current block height (is sent to new block topic) confirmAt blocks (default: 10)
  --subscriber.coolDownDuration value                coolDownDuration is used to sleep for a while when a channel reaches its size (default: 0)
  --subscriber.internalTransactionEventTopic value   topic name that internal transaction message will be published to
...
PROXY NODE OPTIONS:
  --rpcUrl value                      rpcUrl is used in Proxy mode to forward rpc requests
  --freeGasProxyUrl value             freeGasProxyUrl is used in Proxy mode to forward free gas transaction
  --dbCacheSizeLimit value            dbCacheSizeLimit is used in Proxy mode to set cache size limit of the database (default: 0)
